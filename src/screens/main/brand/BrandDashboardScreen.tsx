...
// ─── Brand Dashboard ──────────────────────────────────────────────
type Tab = 'home' | 'match' | 'likes' | 'messages' | 'profile';
  ...
  return (
    <View style={s.root}>
      {viewingProfileId ? (
       ...
       activeTab === 'match' ? <BrandSwipeScreen onViewProfile={(id) => setViewingProfileId(id)} onNavigateToMessages={() => setActiveTab('messages')} /> :
       activeTab === 'likes' ? <LikesScreen /> :
       activeTab === 'messages' ? <ChatScreen onConversationStateChange={setIsConversationOpen} /> :
       <BrandProfileScreen />}
